    ...
    try {
        ...
        foreach ($scores as $catId => $score) {
            ...
            if ($comp) {
                ...
                // 2.5 Record Skill Development Progress
                // Get previous level
                $prevLevelStmt = $pdo->prepare("SELECT level FROM employee_competencies WHERE employee_id = ? AND competency_id = ?");
                $prevLevelStmt->execute([$employeeId, $compId]);
                $prevLevel = $prevLevelStmt->fetchColumn() ?: null;

                $progressType = 'New Competency';
                if ($prevLevel) {
                    if ($levelMap[$level] > $levelMap[$prevLevel]) {
                        $progressType = 'Improvement';
                    } else if ($hasGap) {
                        $progressType = 'Persistent Gap';
                    } else {
                        $progressType = 'Improvement'; // Maintenance of high level
                    }
                }

                $insProgress = $pdo->prepare("INSERT INTO skill_development_progress (employee_id, assessment_id, competency_name, previous_level, new_level, progress_type) VALUES (?, ?, ?, ?, ?, ?)");
                $insProgress->execute([$employeeId, $assessmentId, $catName, $prevLevel, $level, $progressType]);
