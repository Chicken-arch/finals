...
<div class="p-8">
    ...
    <?php if ($activeTab == 'leave'): ?>
            ...
            <?php if (isset($_GET['leave_submitted'])): ?>
                ...
                <?php else: ?>
                    <div class="mb-4 p-4 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm">
                        There was a problem submitting your leave request. Please check the fields and try again.
                    </div>
                <?php endif; ?>
            <?php endif; ?>
                ...
                <?php foreach ($myLeaves as $leave): ?>
                    <div class="border border-gray-200 rounded-lg p-6">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <?php if ($isAdmin && !empty($leave['employee_name'])): ?>
                                        <span class="text-gray-500 text-sm"> &mdash; <?php echo htmlspecialchars($leave['employee_name']); ?></span>
                                    <?php endif; ?>
                                </h3>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
                            ...
                            <div>
                                <p class="text-gray-600 text-sm mb-1">Request Date</p>
                            ...
                            <?php if ($isAdmin && $leave['status'] === 'Pending'): ?>
                                <div class="flex flex-col sm:flex-row gap-2 justify-end">
                                    <form method="POST" action="ess.php?tab=leave" class="inline">
                                        ...
                                        <button type="submit" class="px-3 py-2 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700 w-full sm:w-auto">
                                            Approve
                                        </button>
                                    </form>
                                    <form method="POST" action="ess.php?tab=leave" class="inline">
                                        ...
                                        <button type="submit" class="px-3 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 w-full sm:w-auto">
                                            Reject
                                        </button>
                                    </form>
                                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <?php if (!$isAdmin): ?>
                    <div class="flex gap-2">
                        <?php if ($canClockIn): ?>
                            <form method="POST" action="ess.php?tab=leave">
            ...
            <?php if (isset($_GET['clocked_in'])): ?>
                <div class="mb-4 p-4 bg-green-50 border border-green-200 text-green-800 rounded-lg text-sm">
                    Clocked in successfully at <?php echo date('h:i A'); ?>.
                </div>
            <?php elseif (isset($_GET['clocked_out'])): ?>
                <div class="mb-4 p-4 bg-blue-50 border border-blue-200 text-blue-800 rounded-lg text-sm">
                    Clocked out successfully at <?php echo date('h:i A'); ?>.
                </div>
            <?php endif; ?>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-200">
                            <?php if ($isAdmin): ?>
                                <th class="text-left py-3 px-4 text-gray-700">Employee</th>
                            <?php endif; ?>
                    ...
                    <tbody>
                        <?php foreach ($myAttendance as $record): ?>
                            <tr class="border-b border-gray-100">
                                <?php if ($isAdmin): ?>
                                    <td class="py-3 px-4 text-gray-900 font-medium"><?php echo htmlspecialchars($record['employee_name']); ?></td>
                                <?php endif; ?>
                                ...
                                <td class="py-3 px-4 text-gray-900"><?php echo $record['hours'] ?? '0.00'; ?></td>
                                <td class="py-3 px-4">
